...
// define pin numbers (pico)
const byte ledPins[5] = {9,10,11,12,13};
const byte buttonPins[6] = {3,4,5,6,7,8};
const byte analogPins[4] = {26,27,28,28}; // temp...
...
void setup(){
  ...
  Mozzi.setPin(0,2); // set output to GP2 on Pico
  ...
  Serial1.begin(31250); // begin serial communication for MIDI input/output
  //Serial1.begin(9600);
...
void startBeat() {
  ...
  Serial1.write(0xFA); // MIDI clock start
}

void stopBeat() {
  beatPlaying = false;
  Serial1.write(0xFC); // MIDI clock stop
  syncReceived = false;
}
...
void playMidiNote(byte sampleNum, byte velocity) {
  Serial1.write(midiNoteCommands[sampleNum]); // note down command
  Serial1.write(midiNotes[sampleNum]); // note number
  Serial1.write(velocity>>1); // velocity (scaled down to MIDI standard, 0-127)
}
...
void cancelMidiNotes() {
  ...
  for(i=0; i<NUM_SAMPLES; i++) {
    if(bitRead(noteDown,i)) {
      Serial1.write(midiNoteCommands[i]); // note down command (zero velocity is equivalent to note up)
      Serial1.write(midiNotes[i]); // note number
      Serial1.write(B00000000); // zero velocity
...
AudioOutput_t updateAudio(){
  ...
  AudioOutput_t asig = ((sampleVolumes[0]*sample0.next())>>atten)+
...
void resetSessionToDefaults() {
  ...
  storedValues[RANGE] = 128; // 0
  storedValues[MIDPOINT] = 128;
  
  storedValues[PITCH] = 160; // 160
